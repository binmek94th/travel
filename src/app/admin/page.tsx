...
async function getDashboardData() {
  const [
    ...
    adminDb.collection("tours").where("status", "==", "active").count().get(),
    adminDb.collection("bookings").where("status", "==", "confirmed").count().get(),
    adminDb.collection("bookings").where("status", "==", "pending").count().get(),
    fetchCollection("bookings", { orderBy: ["createdAt", "desc"], limit: 8 }),
    adminDb.collection("operators").where("isVerified", "==", false).count().get(),
    adminDb.collection("reviews").where("isFlagged", "==", true).count().get(),
    adminDb.collection("tours").where("status", "==", "pending").count().get(),
  ]);

  return {
    userCount:        userCount.data().count,
    destCount:        destCount.data().count,
    tourCount:        tourCount.data().count,
    confirmedBookings:confirmedBookings.data().count,
    pendingBookings:  pendingBookings.data().count,
    recentBookings,
    pendingOperators: pendingOperators.data().count,
    flaggedReviews:   flaggedReviews.data().count,
    pendingTours:     pendingTours.data().count,
  };
}
                    ...
                    {data.recentBookings.map((b: any) => (
                      <tr key={b.id} className="hover:bg-slate-50 transition-colors">
                        <td className="px-4 py-3 border-b border-slate-50">
                          <div className="flex items-center gap-2">
                            <div className="w-7 h-7 rounded-full bg-gradient-to-br from-cyan-400 to-cyan-600 flex items-center justify-center text-xs font-bold text-white flex-shrink-0">
                              {(b.travelerId?.[0] ?? "?").toUpperCase()}
                            </div>
                            <span className="text-sm text-slate-700 truncate max-w-[120px]">{b.travelerId ?? "—"}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3 border-b border-slate-50 text-slate-500 text-xs truncate max-w-[140px]">
                          {b.tourId ?? "—"}
                        </td>
                        <td className="px-4 py-3 border-b border-slate-50">
                        ...
                        <td className="px-4 py-3 border-b border-slate-50 font-semibold text-slate-700">
                          {b.currency === "ETB" ? "ETB " : "$"}{b.totalUSD ?? b.totalETB ?? "—"}
                        </td>
