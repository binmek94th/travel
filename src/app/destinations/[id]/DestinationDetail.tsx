...
function DestinationMap({ lat, lng, name }: { lat: number; lng: number; name: string }) {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            const map = L.map(containerRef.current!, {
                ...
                scrollWheelZoom: false, // better UX on a page with scrolling
            });
            mapRef.current = map;
            ...
            const icon = L.divIcon({
                html: `<div style="width:36px;height:44px;filter:drop-shadow(0 4px 8px rgba(14,133,178,0.45))">
          <svg viewBox="0 0 36 44" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <circle cx="18" cy="12" r="3" fill="url(#dg)"/>
            <defs>
          ...
          <div style="font-weight:700;font-size:13px;color:#0A3D52;margin-bottom:2px">${name}</div>
          <div style="font-size:11px;color:#1A6A8A">${lat.toFixed(4)}, ${lng.toFixed(4)}</div>
        </div>`,
    ...
    return (
        ...
        <span className="font-mono text-[0.72rem] text-[#1A6A8A]">
            ...
            <div className="relative h-[55vh] min-h-[360px] overflow-hidden bg-gradient-to-br from-[#EBF8FF] to-[#D6F0FA]">
                ...
                <div className="absolute bottom-0 left-0 right-0 px-6 pb-8 md:px-16">
                    <div className="mx-auto max-w-6xl">
                        <div className="mb-3 flex flex-wrap items-center gap-2">
                            {destination.isHiddenGem && (
                                <span className="rounded-full bg-amber-400/90 px-3 py-0.5 text-xs font-bold text-white">💎 Hidden gem</span>
                            )}
                  ...
                  <span className="text-amber-400">★</span>
                                    {avgRating.toFixed(1)} ({destination.reviewCount} reviews)
                </span>
                            )}
                        ...
                        {allImages.map((img, i) => (
                            <button key={i} onClick={() => setActiveImg(i)}
                                    className={`h-14 w-20 flex-shrink-0 overflow-hidden rounded-lg border-2 transition-all ${
                                        i === activeImg ? "border-[#1E9DC8] opacity-100" : "border-transparent opacity-60 hover:opacity-100"
                                    }`}>
                    ...
                    <div className="lg:col-span-2 flex flex-col gap-10">
                        ...
                        {nearbyTours.length > 0 && (
                            <section>
                                <div className="mb-5 flex items-center justify-between">
                                    <h2 className="text-2xl font-semibold text-[#0A3D52]" style={{ fontFamily:"'Playfair Display',serif" }}>
                                        Tours in {destination.name}
                                    </h2>
                                    <Link href={`/tours?destination=${destination.id}`} className="text-sm font-medium text-[#1E9DC8] hover:underline">
                                        View all
                                    </Link>
                                </div>
                                    ...
                                    {nearbyTours.map(tour => (
                                        <Link key={tour.id} href={`/tours/${tour.id}`}
                                            ...
                                            <div className="relative h-24 w-24 flex-shrink-0 overflow-hidden bg-gradient-to-br from-[#EBF8FF] to-[#D6F0FA]">
                                                ...
                                                {tour.isFeatured && (
                                                    <div className="absolute left-0 top-1 rounded-r bg-[#1E9DC8] px-1.5 py-0.5 text-[0.55rem] font-bold text-white">⭐</div>
                                                )}
                                            </div>
                                            <div className="flex flex-1 flex-col justify-between p-3">
                                                <p className="line-clamp-2 text-sm font-semibold leading-snug text-[#0A3D52]" style={{ fontFamily:"'Playfair Display',serif" }}>
                                                    {tour.title}
                                                </p>
                        ...
                        <section>
                            <h2 className="mb-5 text-2xl font-semibold text-[#0A3D52]" style={{ fontFamily:"'Playfair Display',serif" }}>
                                Reviews
                                {reviews.length > 0 && <span className="ml-2 text-base font-light text-[#1A6A8A]">({reviews.length})</span>}
                            </h2>
                            {reviews.length === 0 ? (
                                ...
                                <div className="flex flex-col gap-4">
                                    <div className="flex items-center gap-6 rounded-xl border border-[rgba(14,133,178,0.10)] bg-[#F8FCFF] p-5">
                                        <div className="text-center flex-shrink-0">
                                            <div className="text-4xl font-bold text-[#0A3D52]" style={{ fontFamily:"'Playfair Display',serif" }}>
                                                {avgRating.toFixed(1)}
                                            </div>
