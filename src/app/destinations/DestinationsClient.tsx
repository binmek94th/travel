...
// ── AUTH — single listener at page level ──────────────────────────────────────
function useAuth() {
...
// ── SAVE BUTTON ───────────────────────────────────────────────────────────────
function SaveBtn({
                     destId, destName, uid, redirectToSignup,
                 }: {
    ...
    useEffect(() => {
        if (!uid) return;
        fetch(`/api/user/saved-destinations?destinationId=${destId}`)
            .then(r => r.json())
    ...
    return (
        <button
            onClick={toggle}
            title={saved ? "Unsave" : uid ? "Save destination" : "Sign up to save"}
            ...
            style={{
                ...
                boxShadow:  saved ? "0 2px 12px rgba(239,68,68,0.40)" : "0 2px 8px rgba(0,0,0,0.18)",
...
function PlaneAnim() {
    ...
    return (
        <>
            {px > 0 && <div style={{ position:"absolute", top:`calc(28% + ${py}px)`, left:0, width:px, height:1.5, background:"linear-gradient(90deg,transparent,rgba(14,133,178,0.18))", borderRadius:2, pointerEvents:"none" }}/>}
...
// ── DESTINATION CARD ──────────────────────────────────────────────────────────
function DestCard({ dest, delay, uid, redirectToSignup }: {
    dest: Destination; delay: number;
    uid: string | null | undefined;
    redirectToSignup: () => void;
}) {
    const { ref, visible } = useReveal();
        ...
        <div ref={ref} style={{
            ...
            <Link
                ...
                style={{
                    ...
                    boxShadow: hovered ? "0 28px 60px rgba(14,133,178,0.18), 0 8px 20px rgba(14,133,178,0.10)" : "0 2px 12px rgba(14,133,178,0.06)",
                    transform: hovered ? `translateY(-10px) rotateX(${tiltX}deg) rotateY(${tiltY}deg) scale(1.01)` : "translateY(0) rotateX(0) rotateY(0) scale(1)",
            ...
            >
                ...
                <div className={`relative flex h-52 items-center justify-center overflow-hidden bg-gradient-to-br ${catGradient} text-5xl`}>
                    ...
                    {/* Hidden gem badge — left side so it doesn't clash with save btn */}
                    {dest.isHiddenGem && (
                        <div className="absolute left-3 top-3 flex items-center gap-1 rounded-full bg-white/90 px-2.5 py-1 text-[0.65rem] font-bold text-amber-700 shadow-sm backdrop-blur-sm"
                             style={{ transform: hovered ? "scale(1.05)" : "scale(1)", transition: "transform 0.2s" }}>
                            💎 Hidden gem
                        </div>
                    )}

                    {/* Category tags — below gem badge if present */}
                    {!dest.isHiddenGem && (
                ...
                <div className="flex flex-1 flex-col p-5">
                    ...
                    <p className="mb-4 line-clamp-2 flex-1 text-sm font-light leading-relaxed text-[#1A6A8A]">{dest.description}</p>
                    <div className="flex items-center justify-between border-t border-[#EBF8FF] pt-3">
                        ...
                        <span className="flex items-center gap-1 rounded-lg px-3 py-1.5 text-xs font-bold transition-all"
                              style={{ background: hovered ? "linear-gradient(135deg,#28B8E8,#0A6A94)" : "#EBF8FF", color: hovered ? "#fff" : "#1E9DC8", transition: "all 0.25s ease" }}>
                            Explore
                            <svg width="11" height="11" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"><path d="M2 6h8M6 2l4 4-4 4"/></svg>
                        </span>
                    </div>
                </div>
            ...
            <div className="sticky top-16 z-40 border-b border-[rgba(14,133,178,0.08)] bg-white/96 px-6 py-3 shadow-[0_2px_20px_rgba(14,133,178,0.07)] backdrop-blur-lg">
                <div className="mx-auto flex max-w-6xl flex-wrap items-center gap-2">
                    ...
                    <button onClick={() => push({ hidden: hiddenGems ? "" : "true" })}
                            className={`flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-sm transition-all ${hiddenGems ? "border-[#1E9DC8] bg-[#EBF8FF] font-semibold text-[#0A3D52]" : "border-[rgba(14,133,178,0.18)] bg-white font-light text-[#1A6A8A] hover:bg-[#EBF8FF]"}`}>
                        💎 Hidden gems
                    </button>
                    ...
                    {destinations.length === 0 ? (
                        <div className="flex flex-col items-center py-28 text-center">
                            <div className="mb-6 flex h-24 w-24 items-center justify-center rounded-full bg-gradient-to-br from-[#EBF8FF] to-[#D6F0FA] text-5xl shadow-[0_8px_32px_rgba(14,133,178,0.12)]"
                                 style={{ animation: "bounce-gentle 3s ease-in-out infinite" }}>📍</div>
                        ...
                        {pageNumbers.map((p, i) =>
                            p === "…" ? (
                                ...
                                <Link key={p} href={`?${buildParams(baseParams, { page: String(p) })}`}
                                      ...
                                      className={`flex h-9 w-9 items-center justify-center rounded-lg border text-sm font-medium transition-all ${p === page ? "border-transparent bg-gradient-to-br from-[#28B8E8] to-[#0A6A94] font-bold text-white shadow-[0_4px_14px_rgba(14,133,178,0.38)]" : "border-[rgba(14,133,178,0.18)] bg-white text-[#1A6A8A] hover:border-[#1E9DC8] hover:bg-[#EBF8FF]"}`}>
                                    {p}
                                </Link>
                            )
                        )}
