...
export function NotificationBell({
    ...
    const buttonClass = open
        ? transparent
            ? "border-transparent bg-white/20 text-cyan-500"
            : "border-cyan-300 bg-cyan-50 text-cyan-600"
        : transparent
            ? "border-transparent bg-transparent text-cyan-500 hover:bg-white/15 hover:text-cyan-400"
    ...
    return (
        <div ref={ref} className="relative">
            <button
                onClick={() => setOpen(v => !v)}
                className={`relative w-10 h-10 rounded-xl border flex items-center justify-center transition-colors ${buttonClass}`}
            >
                ...
                {unread > 0 && (
                    <span className="absolute -top-1 -right-1 min-w-[16px] h-4 px-0.5 bg-red-500 text-white
                                     text-[9px] font-bold rounded-full border-2
                                     flex items-center justify-center leading-none"
                          style={{ borderColor: transparent ? "transparent" : "white" }}>
                        {unread > 9 ? "9+" : unread}
                    </span>
                )}
            </button>
