...
type Event       = { id: string; name: string; type: string; startDate: string; endDate: string; location: string; destName: string; description: string };
type Guide       = { id: string; name: string; regions: string[]; languages: string[]; specialties: string[]; bio: string };

export default function AIPlannerButton({ destinations, tours, routes, events, guides }: {
    destinations: Destination[]; tours: Tour[]; routes: Route[]; events: Event[]; guides: Guide[];
}) {
    ...
    return (
      ...
      `}</style>
            ...
            {!open && (
                <button onClick={() => setOpen(true)} title="AI Journey Planner"
                    ...
                    {hovered && (
                        <div style={{ position:"absolute", right:"calc(100% + 10px)", top:"50%", transform:"translateY(-50%)", background:"linear-gradient(135deg,#0A3D52,#0E85B2)", color:"white", fontSize:"0.72rem", fontWeight:600, padding:"5px 10px", borderRadius:8, whiteSpace:"nowrap", boxShadow:"0 4px 14px rgba(14,133,178,0.30)", pointerEvents:"none" }}>
                            ✨ AI Journey Planner
            ...
            <AIPlannerDialog
                ...
                routes={routes} events={events} guides={guides}
            />
        </>
    );
}