...
const DESTINATIONS_LINKS: NavLink[] = [
    { label: "Tours",       href: "/tours"       },
    { label: "AI Planner",  href: "/ai-planner"  },
    { label: "Community",   href: "/community"   },
];

const TOURS_LINKS: NavLink[] = [
...
function Avatar({ name, photoURL }: { name: string; photoURL?: string | null }) {
    ...
    if (photoURL) {
        return (
            <img
                src={photoURL}
                alt={name}
                style={{ width: 34, height: 34, borderRadius: "50%", objectFit: "cover", border: "2px solid rgba(14,133,178,0.25)" }}
            />
        );
    }
    ...
    return (
        <div style={{
            ...
            border: "2px solid rgba(14,133,178,0.25)",
            flexShrink: 0,
        }}>
            {initials}
        </div>
...
function UserDropdown({ user, onClose }: { user: User; onClose: () => void }) {
    ...
    return (
        <div ref={ref} style={{
            ...
            border: "1px solid rgba(14,133,178,0.12)",
            borderRadius: 14, boxShadow: "0 16px 48px rgba(14,133,178,0.16), 0 4px 12px rgba(14,133,178,0.08)",
            overflow: "hidden", zIndex: 200,
            animation: "dropdown-pop 0.18s cubic-bezier(0.22,1,0.36,1) both",
        }}>
            <style>{`
            ...
            <div style={{ padding: "1rem 1rem 0.75rem", borderBottom: "1px solid rgba(14,133,178,0.08)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "0.65rem" }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                        <div style={{ fontSize: "0.83rem", fontWeight: 700, color: "#0A3D52", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                            {name}
                        </div>
                        <div style={{ fontSize: "0.7rem", color: "#1A6A8A", fontWeight: 300, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                            {email}
                        </div>
                    </div>
                </div>
...
function BookingIcon() {
    return (
        <svg width="15" height="15" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round">
            ...
            <path d="M5 10h2M9 10h2M5 12.5h2"/>
        </svg>
    );
}
...
export default function ContentNav() {
    ...
    return (
        <nav style={{
            ...
            borderBottom: `1px solid ${scrolled ? "rgba(14,133,178,0.12)" : "rgba(14,133,178,0.07)"}`,
            boxShadow: scrolled ? "0 2px 24px rgba(14,133,178,0.09)" : "none",
            ...
            <div style={{ maxWidth: 1280, margin: "0 auto", width: "100%", padding: "0 2rem", display: "flex", alignItems: "center", gap: "1.5rem" }}>
                ...
                <div style={{
                    ...
                    background: `${pageColor}15`,
                    border: `1px solid ${pageColor}30`,
                    borderRadius: 20, padding: "0.25rem 0.75rem",
                    fontSize: "0.72rem", fontWeight: 700,
                    color: pageColor, flexShrink: 0,
                }}>
                    <span style={{ width: 5, height: 5, borderRadius: "50%", background: pageColor, display: "inline-block" }} />
                    {pageLabel}
                </div>
                        ...
                        return (
                            <Link
                                ...
                                style={{
                                    ...
                                    color: active ? "#0A3D52" : "#1A6A8A",
                                    textDecoration: "none",
                                    padding: "0.4rem 0.75rem", borderRadius: 8,
                                    background: active ? "rgba(14,133,178,0.08)" : "transparent",
                            ...
                            >
                                {link.label}
                                {active && (
                                    <span style={{ position:"absolute", bottom:-1, left:"50%", transform:"translateX(-50%)", width:16, height:2, borderRadius:2, background:"linear-gradient(90deg,#28B8E8,#0A6A94)" }}/>
                                )}
                            </Link>
                        );
                    })}
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", flexShrink: 0 }}>
                  ...
                  {name.split(" ")[0]}
                            ...
                            {dropOpen && (
                                <UserDropdown user={user} onClose={() => setDropOpen(false)} />
                            )}
                        </div>
                    ) : (
                        <>
